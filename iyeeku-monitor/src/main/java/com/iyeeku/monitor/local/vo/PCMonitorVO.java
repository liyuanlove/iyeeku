package com.iyeeku.monitor.local.vo;

import com.iyeeku.core.vo.BaseVO;

import java.util.Date;

public class PCMonitorVO extends BaseVO {

    private String zj;

    private String jqbh;

    private String macdz;

    private String dnmc;

    private String czxt;

    private String jqjkzt;

    private String jqzxzt;

    private String sfyxyckz;

    private Date zjyckjsj;

    private Date zjycgjsj;

    private String zxsc;

    private String kzzd1;

    private String kzzd2;

    private String kzzd3;

    public String getZj() {
        return zj;
    }

    public void setZj(String zj) {
        this.zj = zj == null ? null : zj.trim();
    }

    public String getJqbh() {
        return jqbh;
    }

    public void setJqbh(String jqbh) {
        this.jqbh = jqbh == null ? null : jqbh.trim();
    }

    public String getMacdz() {
        return macdz;
    }

    public void setMacdz(String macdz) {
        this.macdz = macdz == null ? null : macdz.trim();
    }

    public String getDnmc() {
        return dnmc;
    }

    public void setDnmc(String dnmc) {
        this.dnmc = dnmc == null ? null : dnmc.trim();
    }

    public String getCzxt() {
        return czxt;
    }

    public void setCzxt(String czxt) {
        this.czxt = czxt == null ? null : czxt.trim();
    }

    public String getJqjkzt() {
        return jqjkzt;
    }

    public void setJqjkzt(String jqjkzt) {
        this.jqjkzt = jqjkzt == null ? null : jqjkzt.trim();
    }

    public String getJqzxzt() {
        return jqzxzt;
    }

    public void setJqzxzt(String jqzxzt) {
        this.jqzxzt = jqzxzt == null ? null : jqzxzt.trim();
    }

    public String getSfyxyckz() {
        return sfyxyckz;
    }

    public void setSfyxyckz(String sfyxyckz) {
        this.sfyxyckz = sfyxyckz == null ? null : sfyxyckz.trim();
    }

    public Date getZjyckjsj() {
        return zjyckjsj;
    }

    public void setZjyckjsj(Date zjyckjsj) {
        this.zjyckjsj = zjyckjsj;
    }

    public Date getZjycgjsj() {
        return zjycgjsj;
    }

    public void setZjycgjsj(Date zjycgjsj) {
        this.zjycgjsj = zjycgjsj;
    }

    public String getZxsc() {
        return zxsc;
    }

    public void setZxsc(String zxsc) {
        this.zxsc = zxsc == null ? null : zxsc.trim();
    }

    public String getKzzd1() {
        return kzzd1;
    }

    public void setKzzd1(String kzzd1) {
        this.kzzd1 = kzzd1 == null ? null : kzzd1.trim();
    }

    public String getKzzd2() {
        return kzzd2;
    }

    public void setKzzd2(String kzzd2) {
        this.kzzd2 = kzzd2 == null ? null : kzzd2.trim();
    }

    public String getKzzd3() {
        return kzzd3;
    }

    public void setKzzd3(String kzzd3) {
        this.kzzd3 = kzzd3 == null ? null : kzzd3.trim();
    }

}
