package com.iyeeku.ext.staff.vo;

import com.iyeeku.core.vo.BaseVO;

import java.util.Date;

public class PFStaffVO extends BaseVO {

    private String yhbh;

    private String rybh;

    private String euipzj;

    private String rlzyid;

    private String yhdlm;

    private String notesid;

    private String gyh;

    private String yhmc;

    private String xmqp;

    private String ssjg;

    private String ssjgbh;

    private String ssjgdy;

    private String xb;

    private String yhlx;

    private String glyjb;

    private Date rzrq;

    private String jycd;

    private Date sdsj;

    private String grzjlx;

    private String sfzh;

    private Date sr;

    private String dhhm;

    private String yj;

    private String dz;

    private String sdlx;

    private String notes邮箱;

    private String yzbm;

    private String bgsdhzx;

    private String bgsdhip;

    private String czhm;

    private String bgsdz;

    private String jtdh;

    private String zt;

    private String jlzt;

    private String zw;

    private String x;

    private String ywmc;

    private String ygzt;

    private String ygxs;

    private Integer pxh;

    private String cjr;

    private Date cjsj;

    private String zhxgr;

    private Date zhxgsj;

    private String kzzd1;

    private String kzzd2;

    private String kzzd3;

    private String kzzd4;

    private String kzzd5;

    private String kzzd6;

    public String getYhbh() {
        return yhbh;
    }

    public void setYhbh(String yhbh) {
        this.yhbh = yhbh == null ? null : yhbh.trim();
    }

    public String getRybh() {
        return rybh;
    }

    public void setRybh(String rybh) {
        this.rybh = rybh == null ? null : rybh.trim();
    }

    public String getEuipzj() {
        return euipzj;
    }

    public void setEuipzj(String euipzj) {
        this.euipzj = euipzj == null ? null : euipzj.trim();
    }

    public String getRlzyid() {
        return rlzyid;
    }

    public void setRlzyid(String rlzyid) {
        this.rlzyid = rlzyid == null ? null : rlzyid.trim();
    }

    public String getYhdlm() {
        return yhdlm;
    }

    public void setYhdlm(String yhdlm) {
        this.yhdlm = yhdlm == null ? null : yhdlm.trim();
    }

    public String getNotesid() {
        return notesid;
    }

    public void setNotesid(String notesid) {
        this.notesid = notesid == null ? null : notesid.trim();
    }

    public String getGyh() {
        return gyh;
    }

    public void setGyh(String gyh) {
        this.gyh = gyh == null ? null : gyh.trim();
    }

    public String getYhmc() {
        return yhmc;
    }

    public void setYhmc(String yhmc) {
        this.yhmc = yhmc == null ? null : yhmc.trim();
    }

    public String getXmqp() {
        return xmqp;
    }

    public void setXmqp(String xmqp) {
        this.xmqp = xmqp == null ? null : xmqp.trim();
    }

    public String getSsjg() {
        return ssjg;
    }

    public void setSsjg(String ssjg) {
        this.ssjg = ssjg == null ? null : ssjg.trim();
    }

    public String getSsjgbh() {
        return ssjgbh;
    }

    public void setSsjgbh(String ssjgbh) {
        this.ssjgbh = ssjgbh == null ? null : ssjgbh.trim();
    }

    public String getSsjgdy() {
        return ssjgdy;
    }

    public void setSsjgdy(String ssjgdy) {
        this.ssjgdy = ssjgdy == null ? null : ssjgdy.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getYhlx() {
        return yhlx;
    }

    public void setYhlx(String yhlx) {
        this.yhlx = yhlx == null ? null : yhlx.trim();
    }

    public String getGlyjb() {
        return glyjb;
    }

    public void setGlyjb(String glyjb) {
        this.glyjb = glyjb == null ? null : glyjb.trim();
    }

    public Date getRzrq() {
        return rzrq;
    }

    public void setRzrq(Date rzrq) {
        this.rzrq = rzrq;
    }

    public String getJycd() {
        return jycd;
    }

    public void setJycd(String jycd) {
        this.jycd = jycd == null ? null : jycd.trim();
    }

    public Date getSdsj() {
        return sdsj;
    }

    public void setSdsj(Date sdsj) {
        this.sdsj = sdsj;
    }

    public String getGrzjlx() {
        return grzjlx;
    }

    public void setGrzjlx(String grzjlx) {
        this.grzjlx = grzjlx == null ? null : grzjlx.trim();
    }

    public String getSfzh() {
        return sfzh;
    }

    public void setSfzh(String sfzh) {
        this.sfzh = sfzh == null ? null : sfzh.trim();
    }

    public Date getSr() {
        return sr;
    }

    public void setSr(Date sr) {
        this.sr = sr;
    }

    public String getDhhm() {
        return dhhm;
    }

    public void setDhhm(String dhhm) {
        this.dhhm = dhhm == null ? null : dhhm.trim();
    }

    public String getYj() {
        return yj;
    }

    public void setYj(String yj) {
        this.yj = yj == null ? null : yj.trim();
    }

    public String getDz() {
        return dz;
    }

    public void setDz(String dz) {
        this.dz = dz == null ? null : dz.trim();
    }

    public String getSdlx() {
        return sdlx;
    }

    public void setSdlx(String sdlx) {
        this.sdlx = sdlx == null ? null : sdlx.trim();
    }

    public String getNotes邮箱() {
        return notes邮箱;
    }

    public void setNotes邮箱(String notes邮箱) {
        this.notes邮箱 = notes邮箱 == null ? null : notes邮箱.trim();
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm == null ? null : yzbm.trim();
    }

    public String getBgsdhzx() {
        return bgsdhzx;
    }

    public void setBgsdhzx(String bgsdhzx) {
        this.bgsdhzx = bgsdhzx == null ? null : bgsdhzx.trim();
    }

    public String getBgsdhip() {
        return bgsdhip;
    }

    public void setBgsdhip(String bgsdhip) {
        this.bgsdhip = bgsdhip == null ? null : bgsdhip.trim();
    }

    public String getCzhm() {
        return czhm;
    }

    public void setCzhm(String czhm) {
        this.czhm = czhm == null ? null : czhm.trim();
    }

    public String getBgsdz() {
        return bgsdz;
    }

    public void setBgsdz(String bgsdz) {
        this.bgsdz = bgsdz == null ? null : bgsdz.trim();
    }

    public String getJtdh() {
        return jtdh;
    }

    public void setJtdh(String jtdh) {
        this.jtdh = jtdh == null ? null : jtdh.trim();
    }

    public String getZt() {
        return zt;
    }

    public void setZt(String zt) {
        this.zt = zt == null ? null : zt.trim();
    }

    public String getJlzt() {
        return jlzt;
    }

    public void setJlzt(String jlzt) {
        this.jlzt = jlzt == null ? null : jlzt.trim();
    }

    public String getZw() {
        return zw;
    }

    public void setZw(String zw) {
        this.zw = zw == null ? null : zw.trim();
    }

    public String getX() {
        return x;
    }

    public void setX(String x) {
        this.x = x == null ? null : x.trim();
    }

    public String getYwmc() {
        return ywmc;
    }

    public void setYwmc(String ywmc) {
        this.ywmc = ywmc == null ? null : ywmc.trim();
    }

    public String getYgzt() {
        return ygzt;
    }

    public void setYgzt(String ygzt) {
        this.ygzt = ygzt == null ? null : ygzt.trim();
    }

    public String getYgxs() {
        return ygxs;
    }

    public void setYgxs(String ygxs) {
        this.ygxs = ygxs == null ? null : ygxs.trim();
    }

    public Integer getPxh() {
        return pxh;
    }

    public void setPxh(Integer pxh) {
        this.pxh = pxh;
    }

    public String getCjr() {
        return cjr;
    }

    public void setCjr(String cjr) {
        this.cjr = cjr == null ? null : cjr.trim();
    }

    public Date getCjsj() {
        return cjsj;
    }

    public void setCjsj(Date cjsj) {
        this.cjsj = cjsj;
    }

    public String getZhxgr() {
        return zhxgr;
    }

    public void setZhxgr(String zhxgr) {
        this.zhxgr = zhxgr == null ? null : zhxgr.trim();
    }

    public Date getZhxgsj() {
        return zhxgsj;
    }

    public void setZhxgsj(Date zhxgsj) {
        this.zhxgsj = zhxgsj;
    }

    public String getKzzd1() {
        return kzzd1;
    }

    public void setKzzd1(String kzzd1) {
        this.kzzd1 = kzzd1 == null ? null : kzzd1.trim();
    }

    public String getKzzd2() {
        return kzzd2;
    }

    public void setKzzd2(String kzzd2) {
        this.kzzd2 = kzzd2 == null ? null : kzzd2.trim();
    }

    public String getKzzd3() {
        return kzzd3;
    }

    public void setKzzd3(String kzzd3) {
        this.kzzd3 = kzzd3 == null ? null : kzzd3.trim();
    }

    public String getKzzd4() {
        return kzzd4;
    }

    public void setKzzd4(String kzzd4) {
        this.kzzd4 = kzzd4 == null ? null : kzzd4.trim();
    }

    public String getKzzd5() {
        return kzzd5;
    }

    public void setKzzd5(String kzzd5) {
        this.kzzd5 = kzzd5 == null ? null : kzzd5.trim();
    }

    public String getKzzd6() {
        return kzzd6;
    }

    public void setKzzd6(String kzzd6) {
        this.kzzd6 = kzzd6 == null ? null : kzzd6.trim();
    }

}
